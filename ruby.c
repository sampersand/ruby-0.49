...
char *inplace = __r47_Qnil_to_NULL;
char *strdup();
char *strstr();
char *index();
    ...
    char ***argvp;
{
	...
	switch (c) {
	  ...
	  case 'R':
		...
		if (*p) {
		    ...
		    int i = strtoul(optarg, __r47_Qnil_to_NULL, 8);
    ...
    if (argv[0] == Qnil) return __r47_Qnil_to_NULL;
	...
	while (p < pend) {
	    ...
	    *s = '\0';
		...
		if (p = strstr(p, "ruby -")) {
		    ...
		    argc = 2; argv[0] = __r47_Qnil_to_NULL; argv[1] = p + 5;
