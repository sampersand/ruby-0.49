    ...
    ID id;
{
    ...
    if (st_lookup(class_tbl, id, __r49_cast_to_charpp(VALUE, &body))) {
    ...
    ID id;
{
    ...
    if (!st_lookup(global_tbl, id, __r49_cast_to_charpp(struct global_entry *, &entry))) {
	entry = ALLOC(struct global_entry);
	st_insert(global_tbl, id, __r49_cast_to_charp(struct global_entry, entry));
	entry->id = id;
    ...
    ID id;
{
    ...
    if (st_lookup(class_tbl, id, __r49_cast_to_charpp(VALUE, &class)))
	return class;
    return Qnil;
}
    ...
    ID id;
{
    ...
    if (obj->iv_tbl == Qnil)
	return Qnil;
    if (st_lookup(obj->iv_tbl, id, __r49_cast_to_charpp(VALUE, &val)))
	return val;
    return Qnil;
}
    ...
    ID id;
{
    ...
    if (st_lookup(class_tbl, id, __r49_cast_to_charpp(VALUE, &val))) return val;
    return Qnil;
}
    ...
    ID id;
{
    ...
    while (class) {
	if (class->c_tbl && st_lookup(class->c_tbl, id, __r49_cast_to_charpp(VALUE, &value))) {
	    return value;
	}
	class = class->super;
    }
    ...
    struct RString *name;
{
    ...
    switch (id & ID_SCOPE_MASK) {
      case ID_GLOBAL:
	if (st_lookup(global_tbl, id, __r49_cast_to_charpp(struct global_entry *, &entry)) && entry->mode != GLOBAL_UNDEF)
	    return TRUE;
	break;
